...
from flexgen.cache_selection_controller_quest import CacheManager, select_kv
import json
import sys
import random
...
def print_gpu_memory():
    ...
    for i in range(device_count):
        ...
        print(f"GPU {i}: "
              ...
              f"Free: {mem_info.free / 1024**2:.2f} MB")
...
def set_seed(seed=42):
    ...
    torch.cuda.manual_seed(seed)       # 固定当前 GPU 的随机种子
    torch.cuda.manual_seed_all(seed)   # 固定所有 GPU 的随机种子（多卡时）
    np.random.seed(seed)               # 固定 NumPy 随机种子
    random.seed(seed)                  # 固定 Python 内置 random 模块的随机种子
    torch.backends.cudnn.deterministic = True   # 保证卷积算法确定性
    torch.backends.cudnn.benchmark = False      # 关闭自动寻找最优卷积算法（牺牲速度换确定性）
...
class SelfAttention:
    def __init__(self, config, env, policy, layer_id, enable_prefetching, cache_manager, partial_weight_ratio=0.2, alpha=4, max_num_kv=400):
        ...
        self.pad_idx = None
        self.unhit_id_map = None # use for one time and del
        ...
        self.quest_topk = int(max_num_kv//self.page_size)
    ...
    def init_cache_one_gpu_batch(self, cache_home):
        ...
        if self.layer_id > 1:
            real_max_num_kv = (self.quest_topk+1) * self.page_size
            self.prefetch_kv = device.allocate((2, real_max_num_kv, cache_home.val[0].shape[1], cache_home.val[0].shape[2]), np.float16, pin_memory=True)
    ...
    def prefetch_cache(self, cache_home, cache_read_buf, i, prefetch_idx, prefetch_cache_stream):
        ...
        if path == 0:  # Direct copy
            ...
            if self.policy.attn_sparsity >= 1.0:
                ...
                # print(f"prefetch_idx shape = {real_prefetch_idx.shape}", flush=True)
                
                self.prefetch_kv.data[0, :real_prefetch_idx.shape[0]], self.prefetch_kv.data[1, :real_prefetch_idx.shape[0]] = select_kv(real_prefetch_idx, k_home.data, v_home.data)
                k_c = TorchTensor((real_prefetch_idx.shape[0], k_home.shape[1], k_home.shape[2]), k_home.dtype, self.prefetch_kv.data[0, :real_prefetch_idx.shape[0]], k_home.device)
                v_c = TorchTensor((real_prefetch_idx.shape[0], v_home.shape[1], v_home.shape[2]), v_home.dtype, self.prefetch_kv.data[1, :real_prefetch_idx.shape[0]], v_home.device)
...
#         print(f"################## Self Attention Layer #{self.layer_id} Token #{i}", flush=True)
#         print(f"Is warmup", flush=True)
#         print_gpu_memory()
        ...
        if i == 0:  # prefill
            ...
            # quest append prefill k cache
            if (self.quest_selector is not None) and (not warmup):
            ...
            cur_unhit_id_map = self.unhit_id_map
            ...
            if self.enable_prefetching:
                ...
                h, new_k_cache, new_v_cache, self.prefetch_idx = self.compute.mha_gen(h, mask, w_q,
                    ...
                    self.policy.compress_cache, self.policy.comp_cache_config, self.quest_selector, self.quest_topk, speculation_stream, self.alpha, self.max_num_kv)
                    
                # new_prefetch_idx = self._cache_manager.cache_miss_check(self.layer_id+1, self.prefetch_idx[0])
                # self.prefetch_idx = (new_prefetch_idx, self.prefetch_idx[1])
            else:
                # qin change
                k_cache = group_k_cache
                v_cache = group_v_cache
            ...
            # remove the old unhit_id_map
            group_k_cache = None
            group_v_cache = None
            self.unhit_id_map = None
            
            # TODO 改成append decode kv
            if self.quest_selector is not None and (self.layer_id > 1):
                self.quest_selector.append_decode_kv(new_k_cache.data)
...
class OptLM:
    def __init__(self,
        ...
        # ############################## qin change
        # # continue group
        # path = "/root/sparse-load/script/opt_6.7b_one_group_ids.json"
        ...
        self.head_num = self.config.n_head
        self.head_dim = int(self.config.hidden_size / self.config.n_head)
        self.dtype = torch.float16
        
#         if hasattr(self.config, "num_key_value_head"):
#             self.head_num = self.config.num_key_value_heads
#         elif hasattr(self.config, "num_attention_heads"):
#             self.head_num = self.config.num_attention_heads
#         else:
#             print("config parameter has no num_key_value_heads and num_attention_heads", flush=True)
#             self.head_num = 32
            
#         if hasattr(self.config, "head_dim"):
#             self.head_dim = self.config.head_dim
#         else:
#             self.head_dim = int(self.config.hidden_size/self.head_num)
...
#             self.torch_dtype = torch.float16
        ...
        #     head_dim=self.head_dim,
        #     dtype=torch.float16,
        #     gpu_cache_pred=gpu_cache_pred,
        #     cpu_cache_pred=cpu_cache_pred
        # )
        ############################## qin change
        ...
        for i in range(self.config.num_hidden_layers):
            if policy.sep_layer:
                if (i == 0) or (i == (self.config.num_hidden_layers - 1)):
                    layers.append(SelfAttention(self.config, self.env, self.policy, i, False, None, partial_weight_ratio, alpha, max_num_kv))
                else:
                    layers.append(SelfAttention(self.config, self.env, self.policy, i, True, None, partial_weight_ratio, alpha, max_num_kv))
                self.attn_layer.append(len(layers) - 1)
                layers.append(MLP(self.config, self.env, self.policy, i))
            else:
                layers.append(TransformerLayer(self.config, self.env, self.policy, i, None))
                self.attn_layer.append(len(layers) - 1)
...
def run_flexgen(args):
    ...
    print(f"args.gpu_cache_pred = {args.gpu_cache_pred}", flush=True)
    print(f"args.cpu_cache_pred = {args.cpu_cache_pred}", flush=True)
    print(f"args.gpu_cache_num = {args.gpu_cache_num}", flush=True)
    
    # use_gpu_cache = args.use_gpu_cache
    gpu_cache_num = args.gpu_cache_num
    if gpu_cache_num <= 0:
        tag = 'cpu'
    elif gpu_cache_num >= opt_config.num_hidden_layers-2:
        tag = 'gpu'
    else:
        tag = f'Hybrid(gpu={gpu_cache_num})'
        
    
    # use_gpu_cache=False
#     for l in range(2, opt_config.num_hidden_layers):
#         cur_use_gpu = False
#         if gpu_cache_num > 0:
#             cur_use_gpu = True
#             gpu_cache_num -= 1
...
#         # 保存 profile
#         # prof.export_chrome_trace(f'/root/sparse-load/SparseCache/speedup/profile_mycache_flat_gpu_b{args.gpu_batch_size}_i{prompt_len}_o{gen_len}.json')
        
#         prof.export_chrome_trace(f'/root/sparse-load/SparseCache/speedup/profile_mycache_cpu_b{args.gpu_batch_size}_i{prompt_len}_o{gen_len}.json')
...
def add_parser_arguments(parser):
    ...
    # parser.add_argument("--use-gpu-cache", type=str2bool, default=False)
