...
class SelfAttention:
    ...
    def init_cache_one_gpu_batch(self, cache_home: ValueHolder):
        ...
        if (device.device_type == DeviceType.CPU and
                isinstance(k_cache, TorchTensor) and k_cache.dtype == torch.bfloat16):
            k_cache = TorchTensor.create_from_torch(k_cache.data.to(torch.float16), device)
            v_cache = TorchTensor.create_from_torch(v_cache.data.to(torch.float16), device)
