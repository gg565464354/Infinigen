...
def get_rotary_position_embeddings(seq_len, head_dim, device, dtype):
...
def repeat_kv_cache(x, n_rep, n_kv_head):
    if n_rep == 1:
        return x
...
class TorchDevice:
    ...
    def qwen_input_embed(self, inputs, attention_mask, w_token, pad_token_id, donate):
        if w_token.device.device_type == DeviceType.COMPRESSED:
            w_token = w_token.device.decompress(w_token)
    ...
    def qwen_mlp(self, inputs, w_gate, w_up, w_down, w_ln, donate, eps):
        ...
        hidden = rms_norm(inputs.data, weight=w_ln.data, eps=eps)
        gate = F.linear(hidden, w_gate.data)
        up = F.linear(hidden, w_up.data)
        out = gate * F.silu(up)
    ...
    def init_cache_one_gpu_batch_qwen(self, config, task, policy):
        num_kv_head = getattr(config, "num_key_value_heads", config.num_attention_heads)
        head_dim = getattr(config, "head_dim", config.hidden_size // config.num_attention_heads)
        prompt_len, gen_len, gpu_batch_size = (task.prompt_len, task.gen_len,
                                              policy.gpu_batch_size)
...
#         return TorchTensor.create_from_torch(value, self), k, v
    ...
    def mha(self, inputs, attention_mask, w_q, b_q, w_k, b_k, w_v, b_v,
        ...
        b, s, h = inputs.shape
        head_dim = h // n_head
        # scaling = head_dim ** -0.5  # SDPA 内部自动处理，无需手动

        hidden = F.layer_norm(inputs.data, (h,), weight=w_ln.data, bias=b_ln.data)

        # shape: (b, s, h)
        # 注意：原代码中 q 乘了 scaling，SDPA 会自动做，所以这里移除 * scaling
        ...
        # 保持缓存格式不变（与原代码一致）
        # 原始 k: (b * n_head, head_dim, s) → permute(2,0,1) → (s, b * n_head, head_dim)
        k = k.permute(0, 2, 3, 1).reshape(b * n_head, head_dim, s)
        v = v.permute(0, 2, 1, 3).reshape(b * n_head, s, head_dim)
    ...
    def mha_qwen(self, inputs, attention_mask, w_q, w_k, w_v,
                 w_out, w_ln, q_ln, k_ln, n_head, donate, compress_cache,
                 comp_config, eps, head_dim, num_key_value_groups, num_key_value_heads):
        """Multi-head attention for Qwen (prefill phase)."""
        if w_q.device.device_type == DeviceType.COMPRESSED:
            w_q = w_q.device.decompress(w_q)
            w_k = w_k.device.decompress(w_k)
            w_v = w_v.device.decompress(w_v)
        ...
        if q_ln is not None:
            q = rms_norm(q, weight=q_ln.data, eps=eps)
        if k_ln is not None:
        ...
        cos, sin = get_rotary_position_embeddings(s, head_dim, inputs.data.device, inputs.data.dtype)
        ...
        k_cache = k_before_repeat.permute(2, 0, 1, 3).reshape(
            s, b * num_key_value_heads, head_dim)
        v_cache = v_before_repeat.permute(2, 0, 1, 3).reshape(
            s, b * num_key_value_heads, head_dim)
    ...
    def mha_gen_qwen(self, inputs, attention_mask, w_q, w_k, w_v,
                     w_out, w_ln, q_ln, k_ln, n_head, k_cache, v_cache,
                     donate, attn_sparsity, compress_cache, comp_config,
                     eps, head_dim, num_key_value_groups, num_key_value_heads):
        """Multi-head attention for Qwen (decoding phase)."""
        if w_q.device.device_type == DeviceType.COMPRESSED:
            w_q = w_q.device.decompress(w_q)
            w_k = w_k.device.decompress(w_k)
            w_v = w_v.device.decompress(w_v)
        ...
        cos, sin = get_rotary_position_embeddings(src_s, head_dim, inputs.data.device, inputs.data.dtype)
        cos = cos[src_s - 1:src_s]
        sin = sin[src_s - 1:src_s]
        q, k = apply_rotary_pos_emb(q, k, cos, sin, unsqueeze_dim=1)

        q = q.reshape(b * n_head, tgt_s, head_dim)
        k_new = k.permute(2, 0, 1, 3).reshape(tgt_s, b * num_key_value_heads, head_dim)
        v_new = v.permute(2, 0, 1, 3).reshape(tgt_s, b * num_key_value_heads, head_dim)

        if not isinstance(k_cache, TorchTensor):
            raise NotImplementedError("Mixed device attention not supported for Qwen cache")

        if attn_sparsity < 1.0:
            raise NotImplementedError("Sparse attention not supported for Qwen cache")

        if compress_cache:
            k_all = k_cache.device.decompress(k_cache)[:src_s]
            v_all = v_cache.device.decompress(v_cache)[:src_s]
        else:
            k_all = k_cache.data[:src_s]
            v_all = v_cache.data[:src_s]

        k_all[src_s - 1:src_s] = k_new
        v_all[src_s - 1:src_s] = v_new

        k_all = repeat_kv_cache(k_all, num_key_value_groups, num_key_value_heads)
        v_all = repeat_kv_cache(v_all, num_key_value_groups, num_key_value_heads)

        k_all = k_all.permute(1, 2, 0).reshape(b * n_head, head_dim, src_s)
        v_all = v_all.permute(1, 0, 2).reshape(b * n_head, src_s, head_dim)

        if k_all.is_cuda:
            mask = attention_mask.data
            if mask.device != k_all.device:
                mask = mask.to(k_all.device)
            value = self._attention_value(q, k_all, v_all, mask,
            ...
            if mask.is_cuda:
                mask = mask.cpu()
            value = self._attention_value(q_cpu, k_cpu, v_cpu, mask,
