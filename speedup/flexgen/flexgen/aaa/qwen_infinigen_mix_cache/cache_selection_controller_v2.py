...
class CacheManager:
    ...
    def __init__(self, 
        ...
        self._max_hybird_cache_len = max(1, self._max_cpu_cached_len-max_sparse_len)
        ...
        # cpu cache v2: separate GPU cache (fixed size) + CPU cache (for unhit KV)
        self._cpu_gpu_cached_group_kv = {}  # GPU cache part (fixed size, like GPU cache design)
        self._cpu_cpu_cached_group_kv = {}  # CPU cache part (for unhit KV accumulation)
    ...
    def add_cache(self, device, layer_id, batch_size, head_num, sparse_len, hidden_size,
        ...
        if use_gpu_cache:
            ...
            # New CPU cache v2 design: GPU cache part (fixed size) + CPU cache part (for unhit KV)
            # GPU cache part: fixed size (max_sparse_len) - stored in pinned memory, transferred to GPU
            gpu_cache_shape = (self.max_sparse_len, self.bh, self.head_dim)
            gpu_k_cache_space = torch.empty(gpu_cache_shape, dtype=self.dtype, pin_memory=True)
            gpu_v_cache_space = torch.empty(gpu_cache_shape, dtype=self.dtype, pin_memory=True)
            
            # CPU cache part: for unhit KV accumulation
            cpu_cache_shape = (self._max_cpu_cached_len - self.max_sparse_len, self.bh, self.head_dim)
            cpu_k_cache_space = torch.empty(cpu_cache_shape, dtype=self.dtype, pin_memory=True)
            cpu_v_cache_space = torch.empty(cpu_cache_shape, dtype=self.dtype, pin_memory=True)
            
            self._pinned_cached_k_list[layer_id] = [gpu_k_cache_space, cpu_k_cache_space]
            self._pinned_cached_v_list[layer_id] = [gpu_v_cache_space, cpu_v_cache_space]
    ...
    def cpu_cache_load_asyn_v2(self, layer_id, transfer_stream, unhit_idx, pad_idx, all_k, all_v, dtype):
        ...
        # Start GPU transfer for cached KV first to overlap with computation
        ...
        if max_unhit_len > 0:
            # Perform CPU computation while GPU transfer is in progress
            ...
            # Start GPU transfer for unhit KV first to overlap with cache update
            with torch.cuda.stream(transfer_stream):
                unhit_gpu_k = [tmp_unhit_k.cuda(non_blocking=True)]
                unhit_gpu_v = [tmp_unhit_v.cuda(non_blocking=True)]
            
            # Progressive update: concatenate unhit KV to CPU cache
            # Get current cache length
            current_cache_len = group_cached_k[0].shape[0]
            new_total_len = current_cache_len + max_unhit_len
            
            # Check if we need to expand the pinned cache space
            if layer_id in self._pinned_cached_k_list:
                pinned_k_space = self._pinned_cached_k_list[layer_id][0]
                pinned_v_space = self._pinned_cached_v_list[layer_id][0]
                max_pinned_len = pinned_k_space.shape[0]
                
                if new_total_len <= max_pinned_len:
                    # Enough space in pinned memory, concatenate in-place
                    pinned_k_space[current_cache_len:new_total_len].copy_(tmp_unhit_k)
                    pinned_v_space[current_cache_len:new_total_len].copy_(tmp_unhit_v)
                    # Update the cached group KV to point to the expanded cache
                    updated_cached_k = [pinned_k_space[:new_total_len]]
                    updated_cached_v = [pinned_v_space[:new_total_len]]
                    self._cpu_cached_group_kv[layer_id] = (updated_cached_k, updated_cached_v)
                else:
                    # Not enough space, check if we should trigger update
                    if new_total_len > self._max_cpu_cached_len:
                        # Trigger update on next call
    ...
    def cpu_cache_load_asyn_v3(self, layer_id, transfer_stream, prefetch_idx, pad_idx, all_k, all_v, dtype):
        """
        New CPU cache load function: returns (GPU cache, CPU cache, unhit KV).
        - GPU cache: fixed-size cache (like GPU cache design)
        - CPU cache: accumulated unhit KV from previous rounds
        - unhit KV: new unhit KV from current round
        """
        if layer_id not in self._cpu_gpu_cached_group_kv:
            # First time, need to initialize
            group_gpu_k, group_gpu_v, _, _ = self.load_and_update_cpu_cache_v2(
                layer_id, transfer_stream, prefetch_idx, all_k, all_v, dtype
            )
            # Return empty CPU cache and empty unhit KV for first round
            empty_shape = (0, group_gpu_k[0].shape[1], group_gpu_k[0].shape[2])
            empty_k = torch.empty(empty_shape, device=group_gpu_k[0].device, dtype=group_gpu_k[0].dtype)
            empty_v = torch.empty(empty_shape, device=group_gpu_v[0].device, dtype=group_gpu_v[0].dtype)
            # Format: ((gpu_k_list, gpu_v_list), (cpu_k_list, cpu_v_list), (unhit_k, unhit_v))
            return ((group_gpu_k, group_gpu_v), ([empty_k], [empty_v]), (empty_k, empty_v))

        # Get existing caches
        group_gpu_cached_k, group_gpu_cached_v = self._cpu_gpu_cached_group_kv[layer_id]
        group_cpu_cached_k, group_cpu_cached_v = self._cpu_cpu_cached_group_kv[layer_id]

        # Step 1: Check cache miss to identify unhit KV
        cur_unhit_idx = self.cache_miss_check(layer_id, prefetch_idx)
        max_unhit_len = cur_unhit_idx.shape[0]
        
        cur_pad_idx = pad_idx
        if cur_pad_idx is None:
            cur_pad_idx = torch.zeros((1, 1, prefetch_idx.shape[2]), dtype=torch.int32)
        cur_pad_idx = cur_pad_idx.cpu()
        cur_unhit_length = self.layer_unhit_lengths.get(layer_id)
        ...
        # Step 2: Start GPU transfer for GPU cache and CPU cache (overlap with computation)
        with torch.cuda.stream(transfer_stream):
            group_gpu_cached_gpu_k = group_gpu_cached_k
            group_gpu_cached_gpu_v = group_gpu_cached_v
            group_cpu_cached_gpu_k = [k.cuda(non_blocking=True) for k in group_cpu_cached_k]
            group_cpu_cached_gpu_v = [v.cuda(non_blocking=True) for v in group_cpu_cached_v]

        # Step 3: Get unhit KV (CPU computation while GPU transfer is in progress)
        if max_unhit_len > 0:
            _C.select_kv_tensor_with_pad(
                cur_unhit_idx,
                cur_unhit_length_list,
            ...
            # Step 4: Start GPU transfer for unhit KV (overlap with cache update)
            with torch.cuda.stream(transfer_stream):
                unhit_gpu_k = tmp_unhit_k.cuda(non_blocking=True)
                unhit_gpu_v = tmp_unhit_v.cuda(non_blocking=True)
            
            # Step 5: Progressive update: concatenate unhit KV to CPU cache
            current_cpu_cache_len = group_cpu_cached_k[0].shape[0]
            new_cpu_cache_len = current_cpu_cache_len + max_unhit_len
            
            # Check if we have enough space in CPU cache
            if layer_id in self._pinned_cached_k_list and len(self._pinned_cached_k_list[layer_id]) > 1:
                cpu_pinned_k_space = self._pinned_cached_k_list[layer_id][1]
                cpu_pinned_v_space = self._pinned_cached_v_list[layer_id][1]
                # max_cpu_pinned_len = cpu_pinned_k_space.shape[0]
                
                if new_cpu_cache_len <= self._max_hybird_cache_len:
                    # Enough space, concatenate in-place
                    cpu_pinned_k_space[current_cpu_cache_len:new_cpu_cache_len].copy_(tmp_unhit_k)
                    cpu_pinned_v_space[current_cpu_cache_len:new_cpu_cache_len].copy_(tmp_unhit_v)
                    # Update the cached CPU cache to point to the expanded cache
                    updated_cpu_cached_k = [cpu_pinned_k_space[:new_cpu_cache_len]]
                    updated_cpu_cached_v = [cpu_pinned_v_space[:new_cpu_cache_len]]
                    self._cpu_cpu_cached_group_kv[layer_id] = (updated_cpu_cached_k, updated_cpu_cached_v)
                else:
                    # Not enough space, check if we should trigger update
                    if new_cpu_cache_len > self._max_cpu_cached_len:
                        # Trigger update on next call
                        self._require_update[layer_id] = True
        else:
            unhit_gpu_k = torch.empty((0, group_gpu_cached_gpu_k[0].shape[1], group_gpu_cached_gpu_k[0].shape[2]),
                                      device=group_gpu_cached_gpu_k[0].device, dtype=group_gpu_cached_gpu_k[0].dtype)
            unhit_gpu_v = torch.empty((0, group_gpu_cached_gpu_v[0].shape[1], group_gpu_cached_gpu_v[0].shape[2]),
                                      device=group_gpu_cached_gpu_v[0].device, dtype=group_gpu_cached_gpu_v[0].dtype)

        # Step 6: Return three parts: GPU cache, CPU cache, unhit KV
        # Format: ((gpu_k_list, gpu_v_list), (cpu_k_list, cpu_v_list), (unhit_k, unhit_v))
        return ((group_gpu_cached_gpu_k, group_gpu_cached_gpu_v), 
                (group_cpu_cached_gpu_k, group_cpu_cached_gpu_v), 
                (unhit_gpu_k, unhit_gpu_v))
    ...
    def load_and_update_cpu_cache_v2(self, layer_id, transfer_stream, prefetch_idx, all_k, all_v, dtype):
        """
        New CPU cache design: fixed-size GPU cache + CPU cache for unhit KV.
        Similar to GPU cache design, but stores GPU cache part separately.
        """
        # Step 1: Initialize pinned spaces if needed
        if layer_id not in self._pinned_cached_k_list:
            # GPU cache part: fixed size (max_sparse_len)
            gpu_cache_shape = (self.max_sparse_len, self.bh, self.head_dim)
            gpu_k_cache_space = torch.empty(gpu_cache_shape, dtype=self.dtype, pin_memory=True)
            gpu_v_cache_space = torch.empty(gpu_cache_shape, dtype=self.dtype, pin_memory=True)
            
            # CPU cache part: for unhit KV accumulation
            cpu_cache_shape = (self._max_cpu_cached_len, self.bh, self.head_dim)
            cpu_k_cache_space = torch.empty(cpu_cache_shape, dtype=self.dtype, pin_memory=True)
            cpu_v_cache_space = torch.empty(cpu_cache_shape, dtype=self.dtype, pin_memory=True)
        ...
        # Step 2: Get pinned cache spaces
        gpu_pinned_k_space = self._pinned_cached_k_list[layer_id][0]
        gpu_pinned_v_space = self._pinned_cached_v_list[layer_id][0]
        cpu_pinned_k_space = self._pinned_cached_k_list[layer_id][1]
        cpu_pinned_v_space = self._pinned_cached_v_list[layer_id][1]
        
        cur_sparse_len = prefetch_idx.shape[0]
        final_cpu_k = [gpu_pinned_k_space[:cur_sparse_len]]
        final_cpu_v = [gpu_pinned_v_space[:cur_sparse_len]]

        # Step 3: Get new KV data
        selected_k, selected_v = select_kv(prefetch_idx, all_k, all_v)
        group_cpu_k = [selected_k]
        group_cpu_v = [selected_v]

        # Step 4: Copy data to pinned tensor
        final_cpu_k[0].copy_(group_cpu_k[0])
        final_cpu_v[0].copy_(group_cpu_v[0])
        ...
        # Step 5: Start GPU transfer (async)
        with torch.cuda.stream(transfer_stream):
            group_gpu_k = [k.cuda(non_blocking=True) for k in final_cpu_k]
            group_gpu_v = [v.cuda(non_blocking=True) for v in final_cpu_v]

        # Step 6: Initialize empty CPU cache for unhit KV
        empty_cpu_k = [cpu_pinned_k_space[:0]]  # Empty slice
        empty_cpu_v = [cpu_pinned_v_space[:0]]  # Empty slice

        # Store GPU cache part (fixed size, like GPU cache design)
        self._cpu_gpu_cached_group_kv[layer_id] = (group_gpu_k, group_gpu_v)
        # Store CPU cache part (empty initially, will accumulate unhit KV)
        self._cpu_cpu_cached_group_kv[layer_id] = (empty_cpu_k, empty_cpu_v)
    ...
    def unified_load_api(self, layer_id, transfer_stream, prefetch_idx, pad_idx, all_k, all_v, dtype):
        if self._use_gpu_cache.get(layer_id, False):
            ...
            # 判断是否需要更新
            if self._require_update[layer_id]:
                # 直接更新cache (use v2 for new design)
                self._update_recode[layer_id] = 0
                self._require_update[layer_id] = False
                group_gpu_k, group_gpu_v, group_cpu_k, group_cpu_v = self.load_and_update_cpu_cache_v2(
                    layer_id, transfer_stream, prefetch_idx, all_k, all_v, dtype
                )
                # Return empty CPU cache and empty unhit KV for first round
                empty_shape = (0, group_gpu_k[0].shape[1], group_gpu_k[0].shape[2])
                ...
                # Use v3 for new design: returns (GPU cache, CPU cache, unhit KV)
                self._update_recode[layer_id] += 1
                gpu_cache, cpu_cache, unhit_kv = self.cpu_cache_load_asyn_v3(
                    layer_id, transfer_stream, prefetch_idx, pad_idx, all_k, all_v, dtype
                )

                # if self._update_recode[layer_id] > self._update_pred:
                #     self._require_update[layer_id] = True

                return gpu_cache, cpu_cache, unhit_kv
