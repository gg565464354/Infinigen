...
class MLP:
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask, cache_write_buf, i, k):
        ...
        chunk_size = int(os.environ.get("FLEXGEN_MLP_CHUNK_SIZE", "1024"))
...
class SelfAttention:
    ...
    def prefetch_cache(self, cache_home, cache_read_buf, i: int, prefetch_idx: torch.Tensor, prefetch_cache_stream: torch.cuda.Stream):
        ...
        if path == 0:
            if self.policy.attn_sparsity >= 1.0:
                ...
                result = self._cache_manager.unified_load_api(
                    self.layer_id, prefetch_cache_stream, prefetch_idx_int, pad_idx_int,
                    k_data, v_data, cache_dtype
                )

                # Handle both old format (3 elements: group_gpu_k, group_gpu_v, group_unhit) 
                # and new format (3 elements: (GPU cache, CPU cache, unhit KV))
                if isinstance(result[0], tuple) and len(result[0]) == 2:
                    # New format: ((gpu_k_list, gpu_v_list), (cpu_k_list, cpu_v_list), (unhit_k, unhit_v))
                    # Don't merge here - store as-is to preserve compute-communication overlap
                    # Merging will happen in _merge_group_cache during attention computation
                    # Store in a way that can be identified later: use a special marker
                    cache_read_buf.store(("_NEW_FORMAT_", result))
                else:
                    # Old format: (group_gpu_k, group_gpu_v, group_unhit)
                    group_gpu_k, group_gpu_v, group_unhit = result
                    cache_read_buf.store(
                        ((group_gpu_k, True),
                         (group_gpu_v, True))
                    )
    ...
    def forward(
        ...
        # Decode
        cache_data = cache_read_buf.pop()
        
        # Handle new format: check for special marker
        if isinstance(cache_data, tuple) and len(cache_data) == 2 and cache_data[0] == "_NEW_FORMAT_":
            # New format: merge GPU cache + CPU cache + unhit KV here (during attention computation)
            _, result = cache_data
            gpu_cache, cpu_cache, unhit_kv = result
            gpu_cache_k, gpu_cache_v = gpu_cache
            cpu_cache_k, cpu_cache_v = cpu_cache
            unhit_k, unhit_v = unhit_kv
            
            def _to_tensor_list(x):
                if isinstance(x, (list, tuple)):
                    return [t.data if isinstance(t, TorchTensor) else t for t in x]
                return [x.data if isinstance(x, TorchTensor) else x]
            
            gpu_k_list = _to_tensor_list(gpu_cache_k)
            gpu_v_list = _to_tensor_list(gpu_cache_v)
            cpu_k_list = _to_tensor_list(cpu_cache_k)
            cpu_v_list = _to_tensor_list(cpu_cache_v)
            unhit_k_tensor = unhit_k.data if isinstance(unhit_k, TorchTensor) else unhit_k
            unhit_v_tensor = unhit_v.data if isinstance(unhit_v, TorchTensor) else unhit_v
            
            # Concatenate all parts here (during attention computation, after communication)
            all_k_parts = gpu_k_list + cpu_k_list
            all_v_parts = gpu_v_list + cpu_v_list
            if unhit_k_tensor.shape[0] > 0:
                all_k_parts.append(unhit_k_tensor)
                all_v_parts.append(unhit_v_tensor)
            
            k_cache_buf = torch.cat(all_k_parts, dim=0) if len(all_k_parts) > 1 else all_k_parts[0]
            v_cache_buf = torch.cat(all_v_parts, dim=0) if len(all_v_parts) > 1 else all_v_parts[0]
        else:
            # Old format: ((group_gpu_k, True), (group_gpu_v, True))
            (k_cache_buf, _), (v_cache_buf, _) = cache_data
            ...
            # Merge for old format
            if not isinstance(k_cache_buf, torch.Tensor):
                k_cache_buf = _merge_group_cache(k_cache_buf)
            if not isinstance(v_cache_buf, torch.Tensor):
