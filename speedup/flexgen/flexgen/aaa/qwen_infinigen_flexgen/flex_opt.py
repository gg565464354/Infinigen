"""
Usage:
python3 -m flexgen.original_qwen.flex_opt \
  --model Qwen/Qwen3-8B --path /path/to/hf_dir \
...
def init_weight_list(weight_specs, policy, env):
    ...
    sizes = [tensor.numel() for tensor, _ in weight_specs]
    sizes_cumsum = np.cumsum(sizes)
    ret = []
    for i, (tensor, name) in enumerate(weight_specs):
        mid_percent = (sizes_cumsum[i] - sizes[i] / 2) / sizes_cumsum[-1]
        home = get_choice(mid_percent * 100, dev_percents, dev_choices)
        shape = tuple(tensor.shape)
        dtype = torch_dtype_to_np_dtype.get(tensor.dtype, np.float16)
        ...
        if tensor_cpu.dtype not in torch_dtype_to_np_dtype:
            tensor_cpu = tensor_cpu.to(torch.float16)

        if not compress:
            weight = home.allocate(shape, dtype, pin_memory=pin_memory)
            weight.load_from_np(tensor_cpu.cpu().numpy().astype(dtype, copy=False))
        else:
...
class InputEmbed:
    ...
    def init_weight(self, weight_home, state_dict):
        if "model.embed_tokens.weight" not in state_dict:
            raise KeyError("Missing weight: model.embed_tokens.weight")
        w_token = state_dict["model.embed_tokens.weight"]
        weights = init_weight_list([(w_token, "model.embed_tokens.weight")],
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        h = self.compute.qwen_input_embed(
            h, mask, w_token, self.config.pad_token_id, donate)
        hidden.val = h
...
class OutputEmbed:
    ...
    def init_weight(self, weight_home, state_dict):
        w_ln = pop_weight(state_dict, "model.norm.weight")
        if "model.embed_tokens.weight" not in state_dict:
            raise KeyError("Missing weight: model.embed_tokens.weight")
        if "lm_head.weight" in state_dict and not getattr(self.config, "tie_word_embeddings", False):
            w_token = pop_weight(state_dict, "lm_head.weight")
        else:
            w_token = state_dict["model.embed_tokens.weight"]
            state_dict.pop("lm_head.weight", None)
        weights = init_weight_list(
            [(w_ln, "model.norm.weight"), (w_token, "lm_head.weight")],
            self.policy, self.env)
        weight_home.store(weights)
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        h = self.compute.qwen_output_embed(
            h, w_ln, w_token, donate,
...
class SelfAttention:
    ...
    def init_weight(self, weight_home, state_dict):
        ...
        w_q = pop_weight(state_dict, prefix + "q_proj.weight")
        w_k = pop_weight(state_dict, prefix + "k_proj.weight")
        w_v = pop_weight(state_dict, prefix + "v_proj.weight")
        w_out = pop_weight(state_dict, prefix + "o_proj.weight")
        w_ln = pop_weight(state_dict, f"model.layers.{self.layer_id}.input_layernorm.weight")

        q_norm_key = prefix + "q_norm.weight"
        k_norm_key = prefix + "k_norm.weight"
        head_dim = getattr(self.config, "head_dim", self.config.hidden_size // self.config.num_attention_heads)
        if q_norm_key in state_dict:
            w_q_norm = pop_weight(state_dict, q_norm_key)
        else:
            w_q_norm = torch.ones(head_dim, dtype=self.config.torch_dtype)
        if k_norm_key in state_dict:
            w_k_norm = pop_weight(state_dict, k_norm_key)
        else:
            w_k_norm = torch.ones(head_dim, dtype=self.config.torch_dtype)

        weight_specs = [
            (w_q, prefix + "q_proj.weight"),
            (w_k, prefix + "k_proj.weight"),
            (w_v, prefix + "v_proj.weight"),
            (w_out, prefix + "o_proj.weight"),
            (w_ln, f"model.layers.{self.layer_id}.input_layernorm.weight"),
            (w_q_norm, q_norm_key),
            (w_k_norm, k_norm_key),
        ]
    ...
    def load_weight(self, weight_home, weight_read_buf, k):
        w_q, w_k, w_v, w_out, w_ln, w_q_norm, w_k_norm = weight_home.val
        if k == 0:
            dst1 = self.weight_load_dst
            dst2 = self.compute
            weight_read_buf.store((
                ...
                w_q_norm.smart_copy(dst2),
                w_k_norm.smart_copy(dst2),
            ))
    ...
    def init_cache_one_gpu_batch(self, cache_home):
        if self.policy.cache_gpu_percent == 100:
            ...
            raise NotImplementedError("Mixed cache placement is not supported for Qwen.")

        if self.policy.compress_cache:
            assert device.device_type != DeviceType.MIXED
            num_kv_head = getattr(self.config, "num_key_value_heads", self.config.num_attention_heads)
            head_dim = getattr(self.config, "head_dim", self.config.hidden_size // self.config.num_attention_heads)
            ...
            k_cache = device.allocate(shape, np.float16, self.policy.comp_cache_config, pin_memory=False)
            v_cache = device.allocate(shape, np.float16, self.policy.comp_cache_config, pin_memory=False)
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        num_key_value_heads = getattr(self.config, "num_key_value_heads", n_head)
        num_key_value_groups = n_head // num_key_value_heads
        head_dim = getattr(self.config, "head_dim", self.config.hidden_size // n_head)
        ...
        if k == self.policy.num_gpu_batches - 1:
            ((w_q, donate[2]), (w_k, donate[3]), (w_v, donate[4]),
             (w_out, donate[5]), (w_ln, donate[6]),
             (w_q_norm, donate[7]), (w_k_norm, donate[8])) = weight_read_buf.pop()
        else:
            ((w_q, _), (w_k, _), (w_v, _),
             (w_out, _), (w_ln, _),
             (w_q_norm, _), (w_k_norm, _)) = weight_read_buf.val

        if i == 0:  # prefill
            mask, donate[1] = attention_mask.val.smart_copy(self.compute)
            h, new_k_cache, new_v_cache = self.compute.mha_qwen(
                h, mask, w_q, w_k, w_v, w_out, w_ln, w_q_norm, w_k_norm,
                n_head, donate, self.policy.compress_cache, self.policy.comp_cache_config,
                self.config.rms_norm_eps, head_dim, num_key_value_groups, num_key_value_heads)
            cache_write_buf.store((new_k_cache, new_v_cache))
        else:  # decoding
            mask, donate[1] = attention_mask.val.smart_copy(self.attention_compute)
            (k_cache, donate[9]), (v_cache, donate[10]) = cache_read_buf.pop()
            h, new_k_cache, new_v_cache = self.compute.mha_gen_qwen(
                h, mask, w_q, w_k, w_v, w_out, w_ln, w_q_norm, w_k_norm, n_head,
...
class MLP:
    ...
    def init_weight(self, weight_home, state_dict):
        ...
        w_gate = pop_weight(state_dict, prefix + "gate_proj.weight")
        w_up = pop_weight(state_dict, prefix + "up_proj.weight")
        w_down = pop_weight(state_dict, prefix + "down_proj.weight")
        w_ln = pop_weight(state_dict, f"model.layers.{self.layer_id}.post_attention_layernorm.weight")

        weight_specs = [
            (w_gate, prefix + "gate_proj.weight"),
            (w_up, prefix + "up_proj.weight"),
            (w_down, prefix + "down_proj.weight"),
            (w_ln, f"model.layers.{self.layer_id}.post_attention_layernorm.weight"),
        ]
        weights = init_weight_list(weight_specs, self.policy, self.env)
        weight_home.store(weights)
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        h = self.compute.qwen_mlp(
            h, w_gate, w_up, w_down, w_ln, donate, self.config.rms_norm_eps)
        hidden.val = h
...
class TransformerLayer:
    ...
    def init_weight(self, weight_home, state_dict):
        ...
        self.attention.init_weight(home1, state_dict)
        self.mlp.init_weight(home2, state_dict)
...
class QwenLM:
    def __init__(self,
        ...
        self.model_state_dict = load_state_dict(path)
    ...
    def init_weight(self, j):
        self.layers[j].init_weight(self.weight_home[j], self.model_state_dict)
...
def get_filename(args):
    model_size = os.path.basename(args.model.rstrip("/"))
    percent = ""
...
def run_flexgen(args):
    ...
    if qwen_config.pad_token_id is None:
        qwen_config.pad_token_id = tokenizer.pad_token_id or 0
    if getattr(qwen_config, "rms_norm_eps", None) is None:
        qwen_config.rms_norm_eps = 1e-6
    if getattr(qwen_config, "num_key_value_heads", None) is None:
        qwen_config.num_key_value_heads = qwen_config.num_attention_heads
    qwen_config.head_dim = getattr(
        qwen_config, "head_dim",
        qwen_config.hidden_size // qwen_config.num_attention_heads
    )
    if getattr(qwen_config, "torch_dtype", None) not in torch_dtype_to_np_dtype:
        qwen_config.torch_dtype = torch.float16
    qwen_config.dtype = torch_dtype_to_np_dtype.get(qwen_config.torch_dtype, np.float16)
    ...
    print(f"###Log name=FleGen input_len={prompt_len} output_len={gen_len} bsz={num_prompts} total={total_latency} prefill={prefill_latency} decode={decode_latency}")
    print("=================================================")

def add_parser_arguments(parser):
    parser.add_argument("--model", type=str, default="Qwen/Qwen3-8B",
        help="Model repo id or local HF dir.")
    parser.add_argument("--path", type=str, default="~/qwen3_weights",
        help="Path to a local HF directory containing Qwen safetensors.")
    ...
    parser.add_argument("--gpu-cache-num", type=int, default=30)
    parser.add_argument("--gpu-cache-pred", type=float, default=2)
    parser.add_argument("--cpu-cache-pred", type=float, default=1.5)
